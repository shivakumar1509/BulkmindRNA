...
type ResultFile = {
  path: string; // relative to results/<sample>, e.g. "1_fastqc/R2054Q_M02_2_fastqc.html"
  name: string;
  is_dir: boolean;
  size: number | null;
...
type hookupProps = {
  sampleId?: string; // e.g. "test"
  apiBaseUrl?: string; // e.g. "http://localhost:8000"
  username?: string; // required by backend

  // request-aware status inputs from client selections / run config
  volcanoComparisons?: string[] | null;
  diseaseOfInterest?: string | null;
  heatmapPathways?: string[] | null;

  /**
   * Optional callback so parent can consume status and keep
   * the existing Completed / Ready / Error badge behavior.
   */
  onStatusChange?: (
...
/**
 * Small helpers – mirror Python logic where possible
 */
...
function normalizePath(p: string): string {
  const raw = (p || "").replace(/\\/g, "/").replace(/^\/+/, "").replace(/^\.\/+/, "");
  return raw;
}
...
/**
 * Backend single file download endpoint
 */
function makeDownloadUrl(
  ...
  return `${base}/api/results/${encodeURIComponent(
    sampleId
  )}/download?path=${encPath}&username=${encUser}`;
}

/**
 * Backend ZIP download endpoint (Requires a backend route to zip directories)
 */
function makeZipDownloadUrl(
  base: string,
  sampleId: string,
  relPath: string,
  username: string
  ...
  return `${base}/api/results/${encodeURIComponent(
    sampleId
  )}/download_zip?path=${encPath}&username=${encUser}`;
}
...
function buildBuckets(files: ResultFile[]): FileBuckets {
  ...
  for (const f of files) {
    ...
    if (
      (p.startsWith("1_fastqc/") || p.startsWith("1a_fastqc_trimmed/")) &&
      [".html", ".zip"].includes(ext)
    ) {
      fastqc.push(f);
      continue;
    }
    ...
    if (
      p.startsWith("3_htseq_count/") &&
      [".txt", ".tsv", ".csv"].includes(ext)
    ) {
      htseqRaw.push(f);
      continue;
    }
    ...
    if (
      (p.startsWith("4_analysis/") ||
        p.startsWith("4a_analysis_outlier_removed/")) &&
...
export function getResultsViewerStepStatus(args: {
  ...
  const diseaseOk =
    !wantsDisease ||
    buckets.pathwayTables.length > 0 ||
    buckets.pathwayFigures.length > 0;
  ...
  return {
    badge: allOk ? "completed" : "error",
    qcOk,
    pathwayOk: pathwayOk && diseaseOk,
    volcanoOk,
    heatmapOk,
    wantsVolcano,
    wantsDisease,
...
/**
 * Collapsible card component, now supports a button on the right of the header
 */
...
const P09ResultsViewer: React.FC<hookupProps> = ({
  sampleId = "test",
  apiBaseUrl,
  username,
  volcanoComparisons,
  ...
  useEffect(() => {
    if (!base) {
      setFiles([]);
      setLoading(false);
      setLoadError("API base URL is not configured (API_BASE missing).");
      return;
    }
    ...
    if (!safeUsername) {
      setFiles([]);
      setLoading(false);
      setLoadError("Username is required for results API.");
      return;
    }

    const abort = new AbortController();
    const url =
      `${base}/api/results/${encodeURIComponent(sampleId)}/files` +
      `?username=${encodeURIComponent(safeUsername)}`;
    ...
    fetch(url, { signal: abort.signal })
      ...
      .catch((err) => {
        ...
        setLoadError(
          `Failed to load results file list for sample '${sampleId}': ${
  ...
  useEffect(() => {
    ...
    onStatusChange(stepStatus.badge, {
      qcOk: stepStatus.qcOk,
      pathwayOk: stepStatus.pathwayOk,
      volcanoOk: stepStatus.volcanoOk,
      heatmapOk: stepStatus.heatmapOk,
      wantsVolcano: stepStatus.wantsVolcano,
      wantsDisease: stepStatus.wantsDisease,
      wantsHeatmap: stepStatus.wantsHeatmap,
      totalFilesCount,
    });
  }, [onStatusChange, stepStatus, totalFilesCount]);

  const renderFolderZipButton = (path: string, label: string = "ZIP") => (
    <a
      href={makeZipDownloadUrl(base, sampleId, path, safeUsername)}
      title={`Download ${path} as ZIP`}
      style={{
        fontSize: 12,
        padding: "4px 8px",
        borderRadius: 6,
  ...
  return (
    <div style={{ marginTop: 8, marginBottom: 24 }}>
      <p style={{ margin: "0 0 8px 0", fontSize: 13, color: "#4b5563" }}>
        Preview processed outputs, figures, and helper CSVs for sample{" "}
        <code>{sampleId}</code>.
      </p>
      ...
      {hasAnyResults && !loading && !loadError && (
        <div style={{ fontSize: 13, marginBottom: 10, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <strong>{totalFilesCount}</strong> files found under{" "}
            <code>results/{sampleId}</code>
          </div>
          {renderFolderZipButton("", "Download All .ZIP")}
        </div>
      )}
      ...
      <Section 
        ...
        defaultOpen={true}
      >
        <Section title="📄  FASTQC reports (trimmed + untrimmed)">
          {buckets.fastqc.length === 0 ? (
            <p style={{ fontSize: 13, color: "#6b7280" }}>
              No FASTQC files found.
            </p>
          ) : (
            <>
              <p style={{ fontSize: 12, color: "#6b7280", marginBottom: 6 }}>
                Combined content from <code>1_fastqc</code> and{" "}
                <code>1a_fastqc_trimmed</code>.
              </p>
              <ul style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
                {buckets.fastqc.map((f) => (
                  ...
                  >
                    ...
                    <a
                      href={makeDownloadUrl(base, sampleId, f.path, safeUsername)}
                      style={{
                        fontSize: 12,
                        padding: "4px 8px",
                        borderRadius: 9999,
        ...
        <Section 
          title="🧾  HTSeq counts" 
          headerRight={renderFolderZipButton("3_htseq_count", "ZIP Raw")}
        >
          <Section title="Raw counts (3_htseq_count)">
            {buckets.htseqRaw.length === 0 ? (
              <p style={{ fontSize: 13, color: "#6b7280" }}>
                No HTSeq raw count files found in <code>3_htseq_count</code>.
              </p>
            ) : (
              <ul style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
          ...
          <Section title="Normalized counts (4_analysis / 4a_analysis_outlier_removed)">
            {buckets.htseqNorm.length === 0 ? (
              <p style={{ fontSize: 13, color: "#6b7280" }}>
                No normalized count files found in <code>4_analysis</code> or{" "}
                <code>4a_analysis_outlier_removed</code>.
              </p>
            ) : (
              <ul style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
        ...
        <Section 
          title="🧬  Differentially Expressed Genes (5_DEGs)"
          headerRight={renderFolderZipButton("5_DEGs", "ZIP 5_DEGs")}
        >
          {buckets.degRoot.length === 0 && buckets.degSubdirs.length === 0 ? (
            <p style={{ fontSize: 13, color: "#6b7280" }}>
              No DEG outputs found under <code>5_DEGs</code>.
            </p>
          ) : (
            <>
        ...
        <Section 
          title="🧭  Pathway enrichment tables (6_AI_enrichment)"
          headerRight={renderFolderZipButton("6_AI_enrichment", "ZIP Tables")}
        >
          {buckets.pathwayTables.length === 0 ? (
            <p style={{ fontSize: 13, color: "#6b7280" }}>
              No pathway enrichment tables found under{" "}
              <code>6_AI_enrichment</code>.
            </p>
          ) : (
            <ul style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
        ...
        <Section title="📂  Logs & metadata snapshots (top-level)">
          {buckets.logsSnapshots.length === 0 ? (
            <p style={{ fontSize: 13, color: "#6b7280" }}>
              No top-level logs or CSV snapshots found in{" "}
              <code>results/{sampleId}</code>.
            </p>
          ) : (
            <ul style={{ listStyle: "none", paddingLeft: 0, margin: 0 }}>
      ...
      <Section 
        title="📊  FIGURES" 
        defaultOpen={true}
        headerRight={renderFolderZipButton("7_Final_figures", "ZIP 7_Final_figures")}
      >
        <p style={{ fontSize: 12, color: "#4b5563", marginTop: 0 }}>
          Thumbnails are capped to {THUMB_LIMIT} per category for responsiveness. Use the ZIP buttons to download all files.
        </p>
        ...
        {buckets.figureCategories.map((cat) => {
          ...
          return (
            <Section key={cat.key} title={`${cat.icon}  ${cat.label} — ${total}`}>
              {total === 0 ? (
                <p style={{ fontSize: 13, color: "#6b7280" }}>
                ...
                <>
                  ...
                  {hasMore && (
                    <p style={{ marginTop: 8, fontSize: 11, color: "#6b7280" }}>
                      Showing first {THUMB_LIMIT} of {total} files. Use the ZIP button on the parent folder to download all.
                    </p>
                  )}
                </>
              )}
          ...
          >
            ...
            >
              <div style={{ fontWeight: 600, fontSize: 14 }}>
                Enlarged preview — {enlargedFigure.name}
              </div>
              <button
                type="button"
                onClick={() => setEnlargedFigure(null)}
                style={{
                  border: "none",
      ...
      {hasAnyResults && (
        <div style={{ 
          ...
          <div>
            ...
            <p style={{ margin: 0, fontSize: 14, color: "#15803d" }}>Get a complete ZIP archive of all outputs, figures, logs, and tables for this sample.</p>
          </div>
          <a 
      ...
      <p style={{ marginTop: 16, fontSize: 11, color: "#6b7280" }}>
        ...
        into <code>results/{sampleId}</code> and that FastAPI can read them.
      </p>
    </div>
  );
};
